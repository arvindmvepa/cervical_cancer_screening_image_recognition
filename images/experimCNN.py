...
network = conv_2d(network, 100, 5, strides=2, activation='relu')
#network = max_pool_2d(network, 3, strides=2)
network = local_response_normalization(network)
network = conv_2d(network, 200, 7, activation='relu')
#network = max_pool_2d(network, 3, strides=2)
network = local_response_normalization(network)
network = conv_2d(network, 200, 10, activation='relu')
network = conv_2d(network, 200, 30, activation='relu')
network = conv_2d(network, 200, 50, activation='relu')
#network = max_pool_2d(network, 3, strides=2)
network = local_response_normalization(network)
network = fully_connected(network, 4000, activation='tanh')
